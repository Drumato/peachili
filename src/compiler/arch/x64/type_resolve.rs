use crate::compiler::{arch::x64, common::frontend::ast as high_ast};
...
) -> Result<HashMap<String, x64::PeachiliType>, Box<dyn std::error::Error>> {
    let mut type_env: HashMap<String, x64::PeachiliType> = Default::default();

    // プリミティブな型はそのまま変換するだけ
    type_env.insert("Int64".to_string(), x64::PeachiliType::Int64);
    type_env.insert("Uint64".to_string(), x64::PeachiliType::Uint64);
    type_env.insert("Noreturn".to_string(), x64::PeachiliType::Noreturn);
    type_env.insert("ConstStr".to_string(), x64::PeachiliType::ConstStr);
    type_env.insert("Boolean".to_string(), x64::PeachiliType::Boolean);
