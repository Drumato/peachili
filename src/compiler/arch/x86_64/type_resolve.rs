use crate::compiler::{arch::x86_64, common::frontend::ast as high_ast};
...
) -> Result<HashMap<String, x86_64::PeachiliType>, Box<dyn std::error::Error>> {
    let mut type_env: HashMap<String, x86_64::PeachiliType> = Default::default();

    // プリミティブな型はそのまま変換するだけ
    type_env.insert("Int64".to_string(), x86_64::PeachiliType::Int64);
    type_env.insert("Uint64".to_string(), x86_64::PeachiliType::Uint64);
    type_env.insert("Noreturn".to_string(), x86_64::PeachiliType::Noreturn);
    type_env.insert("ConstStr".to_string(), x86_64::PeachiliType::ConstStr);
    type_env.insert("Boolean".to_string(), x86_64::PeachiliType::Boolean);
...
pub fn resolve_type(
    raw_type_env: &HashMap<String, high_ast::TopLevelDecl>,
    resolved_type_env: HashMap<String, x86_64::PeachiliType>,
    ref_module_name: &String,
    type_name: &String,
) -> Result<(HashMap<String, x86_64::PeachiliType>, x86_64::PeachiliType), TypeResolveError> {
    match type_name.as_str() {
        // プリミティブな型はそのまま変換するだけ
        "Int64" => Ok((resolved_type_env, x86_64::PeachiliType::Int64)),
        "Uint64" => Ok((resolved_type_env, x86_64::PeachiliType::Uint64)),
        "Noreturn" => Ok((resolved_type_env, x86_64::PeachiliType::Noreturn)),
        "ConstStr" => Ok((resolved_type_env, x86_64::PeachiliType::ConstStr)),
        "Boolean" => Ok((resolved_type_env, x86_64::PeachiliType::Boolean)),
