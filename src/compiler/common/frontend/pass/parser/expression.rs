...
use nom::{
    ...
    character::complete::{
        char as parse_char, multispace0},
    multi::{separated_list0},
    sequence::{delimited, tuple, preceded},
    combinator::{map, value},
};
use super::nom_wrapper::{keyword, ws};

type IResultExpr<'a> = nom::IResult<&'a str, ast::ExprInfo<'a>>;

pub fn expression<'a>(arena: &'a Arena<ast::ExprInfo<'a>>, i: &'a str) -> IResultExpr<'a> {
    minus_operation(arena, i)
}
...
fn boolean_literal(i: &str) -> IResultExpr {
    let (rest, literal_kind) = alt(
        (
            value(ast::ExprKind::True, keyword("true")), 
            value(ast::ExprKind::False, keyword("false")),
        )
    )(i)?;
...
fn integer_literal(i: &str) -> IResultExpr {
    map(ws(integer_literal_string), |s: &str| ast::ExprInfo{ kind: ast::ExprKind::Integer{value: s.parse().unwrap()}})(i)
}

/// "'u' [0-9]+"
fn unsigned_integer_literal(i: &str) -> IResultExpr {
    let (rest, unsigned_integer_string) = ws(preceded(parse_char('u'), integer_literal_string))(i)?;
...
mod expression_parser_test {
    ...
    #[test]
    fn primary_test() {
        let result = primary(" u100 Drumato;");
        assert!(result.is_ok());
    ...
    fn boolean_literal_test() {
        let result = boolean_literal(" true false ;");
        assert!(result.is_ok());
