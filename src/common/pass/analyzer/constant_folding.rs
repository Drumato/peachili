...
fn folding_expr(expr_arena: ast::ExprArena, ast_expr_id: ast::ExNodeId) -> ast::ExpressionNode {
    ...
    match ast_expr.get_kind(){
        ast::ExpressionNodeKind::ADD{lhs:lhs_id, rhs:rhs_id} => {
            ...
            if lhs.is_integer_literal() && rhs.is_integer_literal(){
                return ast::ExpressionNode::new_integer(lhs.get_integer_value() + rhs.get_integer_value() , lhs.get_pos());
            }

            ast_expr
        }
        ast::ExpressionNodeKind::SUB{lhs:lhs_id, rhs:rhs_id} => {
            let lhs = folding_expr(expr_arena.clone(), *lhs_id);
            let rhs = folding_expr(expr_arena.clone(), *rhs_id);
        ...
        ast::ExpressionNodeKind::MUL{lhs:lhs_id, rhs:rhs_id} => {
            let lhs = folding_expr(expr_arena.clone(), *lhs_id);
            let rhs = folding_expr(expr_arena.clone(), *rhs_id);
        ...
        ast::ExpressionNodeKind::DIV{lhs:lhs_id, rhs:rhs_id} => {
            let lhs = folding_expr(expr_arena.clone(), *lhs_id);
            let rhs = folding_expr(expr_arena.clone(), *rhs_id);
        ...
        ast::ExpressionNodeKind::NEG{value: value_id} => {
            ...
            if value.is_integer_literal(){
                return ast::ExpressionNode::new_integer(-value.get_integer_value(), value.get_pos());
            }

            ast_expr
        }
        ast::ExpressionNodeKind::CALL{
            names,
            args
        } => {
