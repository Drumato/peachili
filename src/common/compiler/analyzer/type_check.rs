use crate::common::{ast, tld, option, error::CompileError, peachili_type::Type};
...
pub fn type_check_main(
    ...
    if !main_func_exists {
        CompileError::new(
            TypeErrorKind::MAINFUNCNOTFOUND,
            Default::default(),
        ).output();
        std::process::exit(1);
    }
...
fn type_check_fn(
    ...
    if let Ok(stmt_arena) = function.stmt_arena.lock() {
        for stmt_id in function.stmts.iter() {
            ...
            type_check_stmt(tld_env, type_env.get(&function.name).unwrap(), stmt, function.expr_arena.clone(), target)?;
        }
    }
...
fn type_check_expr(
    ...
    match expr.get_kind() {
        ast::ExpressionNodeKind::INTEGER { value: _ } => Ok(Type::new_int64(type_util::resolve_type_size(
            tld_env, type_util::ForCalcTypeSize::INT64, target,
        ))),
        ast::ExpressionNodeKind::UINTEGER { value: _ } => Ok(Type::new_uint64(type_util::resolve_type_size(
            tld_env, type_util::ForCalcTypeSize::UINT64, target,
        ))),
        ast::ExpressionNodeKind::IDENTIFIER { names } => {
            let full_path = names.join("::");
            Ok(type_env.get(&full_path).unwrap().clone())
        }
        ast::ExpressionNodeKind::BOOLEAN { truth: _ } => Ok(Type::new_boolean(type_util::resolve_type_size(
            tld_env, type_util::ForCalcTypeSize::BOOLEAN, target,
        ))),
        ast::ExpressionNodeKind::STRING { contents: _ } => Ok(Type::new_const_str(type_util::resolve_type_size(
...
mod type_check_tests {
    ...
    fn type_check_main_fn_with_invalid_return_type_test() {
        ...
        let actual = type_check_main_fn(&tld_env, &env, &invalid_return_func, option::Target::X86_64);
    ...
    fn type_check_main_fn_with_invalid_args_test() {
        ...
        if let Err(e) = actual {
            assert_eq!(&TypeErrorKind::MAINFUNCMUSTNOTHAVEANYARGUMENTS, e.get_kind());
        }
    }
    ...
    fn invalid_member_access_test() {
        ...
        let member_type = type_check_expr(&tld_env, &env, expr_arena.clone(), &member_ex, option::Target::X86_64);
        type_check_expr_error_test(member_type, TypeErrorKind::CANNOTACCESSMEMBERWITHNOTANIDENTIFIER {
        ...
        let member_type = type_check_expr(&tld_env, &env, expr_arena.clone(), &member_ex, option::Target::X86_64);
        type_check_expr_error_test(member_type, TypeErrorKind::MEMBERNAMEMUSTBEANIDENTIFIER {
        ...
        let member_type = type_check_expr(&tld_env, &env, expr_arena.clone(), &member_ex, option::Target::X86_64);
        type_check_expr_error_test(member_type, TypeErrorKind::CANNOTACCESSMEMBERWITHNOTASTRUCT {
            struct_node: ast::ExpressionNode::new_identifier(vec!["x".to_string()], Default::default()),
        });
        ...
        let member_type = type_check_expr(&tld_env, &env, expr_arena.clone(), &member_ex, option::Target::X86_64);
        type_check_expr_error_test(member_type, TypeErrorKind::UNDEFINEDSUCHAMEMBER {
            member_node: ast::ExpressionNode::new_identifier(vec!["undefined".to_string()], Default::default()),
        });
    }


    fn type_check_expr_error_test(actual: Result<Type, CompileError<TypeErrorKind>>, expected_error: TypeErrorKind) {
        assert!(actual.is_err());
    ...
    fn type_check_expr_test() {
        ...
        let int_type = type_check_expr(&tld_env, &env, expr_arena.clone(), &int_ex, option::Target::X86_64);
        ...
        let uint_type = type_check_expr(&tld_env, &env, expr_arena.clone(), &uint_ex, option::Target::X86_64);
        ...
        let bool_type = type_check_expr(&tld_env, &env, expr_arena.clone(), &bool_ex, option::Target::X86_64);
        assert!(bool_type.is_ok());
        assert_eq!(Type::new_boolean(8), bool_type.unwrap());

        // identifier
        let ident_ex = ast::ExpressionNode::new_identifier(vec!["x".to_string()], Default::default());
        let ident_type = type_check_expr(&tld_env, &env, expr_arena.clone(), &ident_ex, option::Target::X86_64);
        assert!(ident_type.is_ok());
        assert_eq!(Type::new_int64(8), ident_type.unwrap());

        // string-literal
        let strlit_ex = ast::ExpressionNode::new_string_literal("Drumato".to_string(), Default::default());
        let strlit_type = type_check_expr(&tld_env, &env, expr_arena.clone(), &strlit_ex, option::Target::X86_64);
