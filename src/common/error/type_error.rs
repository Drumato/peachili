    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let s = match self {
            ...
            TypeErrorKind::CANNOTACCESSMEMBERWITHNOTANIDENTIFIER {
                ...
                format!("cannot access member of `{:?}`, its not an identifier", struct_node)
            }
            TypeErrorKind::MEMBERNAMEMUSTBEANIDENTIFIER {
                member_node
            } => {
                format!("member name must be an identifier, but got `{:?}", member_node)
            }
            TypeErrorKind::CANNOTACCESSMEMBERWITHNOTASTRUCT { struct_node } => {
                format!("cannot access member of `{:?}`, its not a struct", struct_node)
            }
            ...
            TypeErrorKind::MAINFUNCMUSTNOTHAVEANYARGUMENTS => "entry point `main` mustn't have any arguments".to_string(),
            TypeErrorKind::MAINFUNCMUSTNOTRETURNANYVALUES => "entry point `main` mustn't return any values".to_string(),
        };
